...
def _looks_like_mojibake(text: str) -> bool:  # Проверяем, похожа ли строка на «битую» кодировку
    markers = ("Ð", "Ñ", "Ã", "Â")  # Характерные символы для UTF-8, прочитанного как Latin-1
    return any(marker in text for marker in markers)  # Возвращаем True, если нашли маркеры


def _fix_mojibake(text: str) -> str:  # Пытаемся восстановить строку с неправильной кодировкой
    if not text:  # Если строка пустая
        return text  # Ничего не меняем
    if not _looks_like_mojibake(text):  # Если строка не похожа на «битую»
        return text  # Возвращаем исходный текст
    try:  # Пробуем восстановить UTF-8 из Latin-1
        repaired = text.encode("latin-1").decode("utf-8")  # Переупаковываем строку обратно в UTF-8
    except UnicodeError:  # Если декодирование не удалось
        return text  # Оставляем исходное значение
    return repaired  # Возвращаем исправленную строку


def _repair_payload_value(value: Any) -> Any:  # Рекурсивно чиним «битые» строки внутри payload
    if isinstance(value, str):  # Если значение — строка
        return _fix_mojibake(value)  # Возвращаем исправленную строку
    if isinstance(value, list):  # Если значение — список
        return [_repair_payload_value(item) for item in value]  # Чиним каждый элемент списка
    if isinstance(value, dict):  # Если значение — словарь
        return {key: _repair_payload_value(item) for key, item in value.items()}  # Чиним все значения словаря
    return value  # Все остальные типы возвращаем без изменений
...
def _append_user_event(payload: dict[str, Any]) -> None:  # Добавляем событие в JSON-файл пользователя
    ...
    normalized_payload = _repair_payload_value(payload)  # Исправляем возможные ошибки кодировки в payload

    event_record = {  # Формируем запись события
        "event_time": _format_event_time(),  # Добавляем время события в формате ДД.ММ.ГГГГ ЧЧ:ММ:СС
        "payload": normalized_payload,  # Сохраняем исправленный payload
